...
> **Send file** action now cover the direct SFTP path, including a local file
> chooser for send. Overlay verbs and deep
...
1. **Transport A.** scp/sftp over an auto-enabled `ControlMaster` socket, with
   ...
   **Partially implemented:** public `send` / `receive` SFTP commands and the
   home-page **Send file** action exist, with a local file chooser for send;
   ControlMaster reuse is still pending.
