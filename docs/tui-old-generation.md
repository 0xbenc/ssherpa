...
This tracked screens that felt visually older than the newer framed workflow
surfaces. The old-generation pass is complete; this file remains as the audit
trail for what was aligned. The current new-generation language is:
...
The non-home surfaces were modernized first, then the homepage picker was
aligned after those call sites proved the shared chooser patterns.

## Completed Deferred Surface

### Home Page Picker

- Surface: main `ssherpa` screen.
- Code: `internal/cli/cli.go`, `internal/ui/picker.go`.
- Original issue: flat full-width picker with ASCII rules, filter band, list
  and preview styling from the older visual language.
- Status: completed after every non-home old-generation surface was aligned.
...
## Completed Non-Homepage Worklist

### 1. Check Results Screen

- Surface: reachability results after `CHECK`.
- Code: `internal/ui/check_results.go`.
- Original issue: flat title/rule/table layout, old footer placement, no framed
  shell.
...
- Original issue: custom flat title/rules/split panels. It is functional, but
...
- Original issue: still used generic `ui.Pick`, so it looked older than the FTB
...
- Original issue: all called `ui.Pick` through `pickAlias` or similar helpers.
...
- Original issue: generic picker for a route-building workflow, visually older
...
- Original issue: generic flat picker menus around otherwise newer form/confirm
  flows.
...
- Original issue: generic flat picker before a results screen.
...
- Code: `internal/cli/tui_features.go`.
- Original issue: generic flat picker for a small menu.
...
- Original issue: generic picker surface and older visual hierarchy.
...
- Original issue: it was the root of the old visual language.
...
- Status: completed. Non-home call sites use dedicated framed choosers; the
  remaining `ui.Pick` usage is the now-framed homepage picker.
