...
func runAuthkeys(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	case "revoke", "unseed":
		return runAuthkeysRevoke(args[1:], stdout, stderr)
	default:
...
func runAuthkeysInteractive(flags authkeysFlags, stdout io.Writer, stderr io.Writer) int {
	...
	for {
		...
		case "revoke":
			if code := runAuthkeysRevokeInteractive(authkeysRevokeFlags{SSHKeygenPath: flags.SSHKeygenPath, Timeout: defaultAuthkeysSeedTimeout, Hops: map[string][]string{}}, stdout, stderr); code != 0 {
				return code
			}
		case "replace":
...
func runAuthkeysCurrentKeysViewer(path string, stderr io.Writer) int {
	for {
		...
		item, ok, err := ui.ChooseManagement(context.Background(), items, ui.ManagementChooserOptions{
			...
			Footer:      "enter view / type filter / arrows move / shift+arrows section / esc back",
		})
		if err != nil {
			fmt.Fprintf(stderr, "ssherpa: authkeys picker failed: %v\n", err)
			return 1
		...
		if err := ui.ShowTextView(context.Background(), ui.TextViewOptions{
			...
			Footer:      "up/down scroll / pgup/pgdn page / esc back to keys",
		}); err != nil {
			fmt.Fprintf(stderr, "ssherpa: authkeys viewer failed: %v\n", err)
			return 1
		}
...
func showAuthkeysCurrentKeysEmpty(path string, diagnostics []authkeys.Diagnostic, stderr io.Writer) error {
	...
	return ui.ShowTextView(context.Background(), ui.TextViewOptions{
		...
		Summary:     path,
		Lines:       lines,
		Footer:      "esc back to authorized_keys manager",
	})
}
...
func authkeysMenuItems() []ui.ManagementItem {
	return []ui.ManagementItem{
		...
		{Kind: ui.ItemConfirmDelete, Token: "revoke", Title: "Remove key from remote hosts", Description: "delete one key from SSH aliases", Group: "Remote", Badge: "revoke", Action: "Remove one selected public key from selected SSH hosts"},
